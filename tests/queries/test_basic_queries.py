...
async def test_find_with_regex(connection):
    third = await Ticket.Q().find_one(name__regex="ird")
    assert third.name == "third"

    starts = await Ticket.Q().find_one(name__startswith="t")
    assert starts._id == third._id

    istarts = await Ticket.Q().find_one(name__istartswith="T")
...
async def test_delete_many(connection):
    deleted = await Ticket.Q().delete_many(name="third")
    assert deleted == 2
