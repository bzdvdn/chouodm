...
from chouodm.types import Relation
...
async def relation_data(event_loop, connection):
    ...
    await publish1.save()
    books = [Book.to_relation(book1_id), book2]
    publish2 = Publish(name="publush2", books=books)
    await publish2.save()
...
async def test_create_object(connection):
    ...
    await author.save()
    book = Book.New(title="lord of the rings", author=author.data)
    await book.save()
