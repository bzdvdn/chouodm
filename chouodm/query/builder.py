...
class Builder(object):
    ...
    async def _make_query(
        ...
        **kwargs,
        ...
        method = getattr(self._collection, method_name)
    ...
    async def find_one(
        ...
        **query,
        ...
        if data is not None:
            ...
            if with_relations_objects and self.odm_manager.__document__.has_relations:
                obj = await self.odm_manager.relation_manager.map_relation_for_single(  # type: ignore
                    obj
                )
            return obj
        return None
    ...
    async def find(
        ...
        **query,
        ...
        if with_relations_objects and self.odm_manager.__document__.has_relations:
            data = await self.odm_manager.relation_manager.map_relation_for_array(data)  # type: ignore
    ...
    def _prepare_update_data(self, **fields) -> tuple:
        ...
        if not any("__set" in f for f in fields):
            raise QueryValidationError("not fields for updating!")
        query_params = {}
        set_values = {}
    ...
    async def _find_with_replacement_or_with_update(
        ...
        **query,
        ...
        Returns:
            Union[Dict, 'Document']: MongoModel or Dict or None
        """
    ...
    async def _aggregate(self, *args, **query) -> SimpleAggregateResult:
        ...
        if not aggregation and not group_by:
            raise QueryValidationError("miss aggregation or group_by")
    ...
    async def raw_query(
        ...
        try:
            ...
            raise QueryValidationError("invalid method name")
    ...
    async def get(
        ...
        obj = await self.find_one(session=session, query=query)
        if not obj:
            raise DocumentDoesNotExist(f"{self.odm_manager.document} does not exist.")
        return obj
    ...
    async def drop_index(
        ...
        raise ODMIndexError(f"invalid index name - {index_name}")
