...
class Aggregate(object):
    ...
    def lookup(
        ...
        if (
            local_field not in self.document_class.__struct_fields__
            and local_field != '_id'
        ):
            raise QueryValidationError(
                f'field - {local_field} not a field from model: {from_.__name__}'
            )
        if foreign_field not in from_.__struct_fields__ and foreign_field != '_id':
            raise QueryValidationError(
    ...
    def unwind(self, unwind: Union[str, dict]) -> 'Aggregate':
        if isinstance(unwind, str) and not unwind.startswith('$'):
            raise QueryValidationError('unwind must be startswith $')
    ...
    def replace_with(self, value: Union[str, dict]) -> 'Aggregate':
        if isinstance(value, str) and not value.startswith('$'):
            raise QueryValidationError('string value must be startwith $')
    ...
    def replace_root(self, new_root: Union[str, dict]) -> 'Aggregate':
        if isinstance(new_root, str) and not new_root.startswith('$'):
            raise QueryValidationError('string new_root must be startwith $')
